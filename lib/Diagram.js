...
export default function Diagram(options, injector) {
  ...
  window.initPhysics = function(debugMode) {
    ...
    window.addRectangle = function(x, y, width, height, isStatic) {
    ...
    var handleShape = function(shape) {
      ...
      if (!isCircle) {
        var rectPhysics = addRectangle(shape.x + shape.width / 2, shape.y + shape.height / 2, shape.width, shape.height, shape.isStatic);
